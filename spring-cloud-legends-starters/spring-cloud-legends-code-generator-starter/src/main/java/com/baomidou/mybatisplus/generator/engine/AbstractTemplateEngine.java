...
public abstract class AbstractTemplateEngine {
    ...
    public AbstractTemplateEngine batchOutput() {
        try {
            ...
            for (TableInfo tableInfo : tableInfoList) {
                ...
                //dto
                String entityDto = tableInfo.getEntityDto();
                if (null != entityDto && null != pathInfo.get(ConstVal.ENTITY_DTO_PATH)) {
                    String entityDtoFile = String.format((pathInfo.get(ConstVal.ENTITY_DTO_PATH) + File.separator + "%s" + suffixJavaOrKt()), entityDto);
                    if (isCreate(FileType.ENTITY_DTO, entityDtoFile)) {
                        writerFile(objectMap, templateFilePath(template.getEntityDto()), entityDtoFile);
                    }
                }
                //addDto
                String entityAddDto = tableInfo.getEntityAddDto();
                if (null != entityAddDto && null != pathInfo.get(ConstVal.ENTITY_ADD_DTO_PATH)) {
                    String entityAddDtoFile = String.format((pathInfo.get(ConstVal.ENTITY_ADD_DTO_PATH) + File.separator + "%s" + suffixJavaOrKt()), entityAddDto);
                    if (isCreate(FileType.ENTITY_ADD_DTO, entityAddDtoFile)) {
                        writerFile(objectMap, templateFilePath(template.getEntityAddDto()), entityAddDtoFile);
                    }
                }

                //updDto
                String entityUpdDto = tableInfo.getEntityUpdDto();
                if (null != entityUpdDto && null != pathInfo.get(ConstVal.ENTITY_UPD_DTO_PATH)) {
                    String entityUpdDtoFile = String.format((pathInfo.get(ConstVal.ENTITY_UPD_DTO_PATH) + File.separator + "%s" + suffixJavaOrKt()), entityUpdDto);
                    if (isCreate(FileType.ENTITY_UPD_DTO, entityUpdDtoFile)) {
                        writerFile(objectMap, templateFilePath(template.getEntityUpdDto()), entityUpdDtoFile);
                    }
                }

                //Vo
                String entityVo = tableInfo.getVo();
                if (null != entityVo && null != pathInfo.get(ConstVal.ENTITY_VO_PATH)) {
                    String entityVoFile = String.format((pathInfo.get(ConstVal.ENTITY_VO_PATH) + File.separator + "%s" + suffixJavaOrKt()), entityVo);
                    if (isCreate(FileType.ENTITY_VO, entityVoFile)) {
                        writerFile(objectMap, templateFilePath(template.getEntityVo()), entityVoFile);
                    }
                }


                //entity mapper
                String entityMapper = tableInfo.getEntityMapper();
                if (null != entityMapper && null != pathInfo.get(ConstVal.ENTITY_MAPPER_PATH)) {
                    String entityMapperFile = String.format((pathInfo.get(ConstVal.ENTITY_MAPPER_PATH) + File.separator + "%s" + suffixJavaOrKt()), entityMapper);
                    if (isCreate(FileType.ENTITY_MAPPER, entityMapperFile)) {
                        writerFile(objectMap, templateFilePath(template.getEntityMapper()), entityMapperFile);
                    }
                }
    ...
    public Map<String, Object> getObjectMap(TableInfo tableInfo) {
        ...
        objectMap.put("entityDto", tableInfo.getEntityDto());
        objectMap.put("entityAddDto", tableInfo.getEntityAddDto());
        objectMap.put("entityUpdDto", tableInfo.getEntityUpdDto());
        objectMap.put("entityMapper", tableInfo.getEntityMapper());
        objectMap.put("vo", tableInfo.getVo());
        objectMap.put("isImportDate", config.getGlobalConfig().isImportDate());
        objectMap.put("isImportBigDecimal", config.getGlobalConfig().isImportBigDecimal());
