...
public class AutoGenerator {
    ...
    protected ConfigBuilder pretreatmentConfigBuilder(ConfigBuilder config) {
        ...
        for (TableInfo tableInfo : tableList) {
            ...
            //查找date
            List<TableField> dateList= tableInfo.getFields().stream().filter(
                    field -> "date".equalsIgnoreCase(field.getPropertyType())
            ).collect(Collectors.toList());

            if(CollectionUtils.isNotEmpty(dateList) && dateList.size()>0){
                config.getGlobalConfig().setImportDate(true);
            }


            //查找 BigDecimal
            List<TableField> bigDecimalList= tableInfo.getFields().stream().filter(
                    field -> "BigDecimal".equalsIgnoreCase(field.getPropertyType())
            ).collect(Collectors.toList());

            if(CollectionUtils.isNotEmpty(bigDecimalList) && bigDecimalList.size()>0){
                config.getGlobalConfig().setImportDate(true);
            }
