...
public class ConfigBuilder {
    ...
    private void handlerPackage(TemplateConfig template, String outputDir, PackageConfig config) {
        ...
        packageInfo.put(ConstVal.ENTITY_DTO, joinPackage(config.getParent(), config.getEntityDto()));
        packageInfo.put(ConstVal.ENTITY_ADD_DTO, joinPackage(config.getParent(), config.getEntityDto()));
        packageInfo.put(ConstVal.ENTITY_UPD_DTO, joinPackage(config.getParent(), config.getEntityDto()));
        packageInfo.put(ConstVal.ENTITY_MAPPER, joinPackage(config.getParent(), config.getEntityMapper()));
        ...
        if (null != configPathInfo) {
            ...
            setPathInfo(pathInfo, template.getEntityDto(), outputDir, ConstVal.ENTITY_DTO_PATH, ConstVal.ENTITY_DTO);
            setPathInfo(pathInfo, template.getEntityAddDto(), outputDir, ConstVal.ENTITY_ADD_DTO_PATH, ConstVal.ENTITY_ADD_DTO);
            setPathInfo(pathInfo, template.getEntityUpdDto(), outputDir, ConstVal.ENTITY_UPD_DTO_PATH, ConstVal.ENTITY_UPD_DTO);
            setPathInfo(pathInfo, template.getEntityMapper(), outputDir, ConstVal.ENTITY_MAPPER_PATH, ConstVal.ENTITY_MAPPER);
            setPathInfo(pathInfo, template.getEntityVo(), outputDir, ConstVal.ENTITY_VO_PATH, ConstVal.VO);
        }
    }
    ...
    private List<TableInfo> processTable(List<TableInfo> tableList, StrategyConfig config) {
        ...
        for (TableInfo tableInfo : tableList) {
            ...
            if (StringUtils.isNotBlank(globalConfig.getEntityDto())) {
                tableInfo.setEntityDto(String.format(globalConfig.getEntityDto(), entityName));
            } else {
                tableInfo.setEntityDto(entityName + ConstVal.ENTITY_DTO);
            }

            if (StringUtils.isNotBlank(globalConfig.getEntityAddDto())) {
                tableInfo.setEntityAddDto(String.format(globalConfig.getEntityAddDto(), entityName));
            } else {
                tableInfo.setEntityAddDto(entityName + ConstVal.ENTITY_ADD_DTO);
            }

            if (StringUtils.isNotBlank(globalConfig.getEntityUpdDto())) {
                tableInfo.setEntityUpdDto(String.format(globalConfig.getEntityUpdDto(), entityName));
            } else {
                tableInfo.setEntityUpdDto(entityName + ConstVal.ENTITY_UPD_DTO);
            }

            if (StringUtils.isNotBlank(globalConfig.getEntityVo())) {
                tableInfo.setVo(String.format(globalConfig.getEntityVo(), entityName));
            } else {
                tableInfo.setVo(entityName + ConstVal.VO);
            }

            if (StringUtils.isNotBlank(globalConfig.getEntityMapper())) {
                tableInfo.setEntityMapper(String.format(globalConfig.getEntityMapper(), entityName));
            } else {
                tableInfo.setEntityMapper(entityName + ConstVal.ENTITY_MAPPER);
            }
